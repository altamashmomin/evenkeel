...
| `propose_action` | MCP two-phase | `action_type` ∈ {`create_rule`, `apply_rules`}; the underlying verb's validation must pass at propose time | AGENT-DESIGN step 4. Parks a frozen payload + computed preview in `pending_actions`; writes no audit row — nothing is executed yet. Records the proposing member (`proposed_by_user`, migration #016) from the authenticated identity. Returns a single-use confirmation token with a ~24-hour expiry |
