...
| `record_sync_run` | UI (ops panel) | — | Audits an in-app "Sync now" trigger (who pressed it, the run's exit status + summary). The data writes themselves are `record_transaction`'s, actor `sync`, exactly as the 06:30 timer path — the panel runs the same script as a subprocess, never a second insert path |
