...
| `propose_action` | MCP two-phase | `action_type` ∈ {`create_rule`, `apply_rules`}; the underlying verb's validation must pass at propose time | AGENT-DESIGN step 4. Parks a frozen payload + computed preview in `pending_actions`; writes no audit row — nothing is executed yet. Records the proposing member (`proposed_by_user`, migration #016) from the authenticated identity. Returns a single-use confirmation token with a ~10-minute expiry |
| `confirm_action` | MCP two-phase | token exists, `status='pending'`, not past `expires_at`, **and the confirming identity is the proposer** (`proposed_by_user`, migration #016 — mismatch refused as not-found so it can't be told from a bogus token; a NULL/legacy proposer is unbound) | AGENT-DESIGN step 4. Marks the pending row `confirmed`, then dispatches the frozen payload to the underlying verb (whose own audit row is the executed-write record). Single-use; a re-confirm is refused by the status check |
...
| `set_item_status` | UI, MCP, Ask | item exists & active; `status` ∈ vocab (`stocked`/`low`/`out`, + `ordered` since #015 = bought, not arrived) | The routine interaction (mark low/out/stocked/ordered) — a direct write in the agent tiers (logged, reversible), like `classify_inflow`. A `oneoff` set `stocked` archives itself (bought → off the list) |
| `restock_items` | UI, Ask | `item_ids` a non-empty list (≤ 100, duplicates collapse); EVERY item exists & active before anything changes (all-or-nothing) | The trip verb (Pantry v2 inc 1): one action marks a bought set `stocked` — each item gets `set_item_status`'s exact edit (shared helper) and its OWN audit row, so the log stays per-item attributable. Never touches money |
| `set_item_store` / `set_item_need_by` / `set_item_snooze` | UI, Ask | item exists & active; store ≤ 60 chars; dates are `YYYY-MM-DD`; blank clears (NULL = the pre-#014 behavior) | Pantry v2 inc 2 metadata setters (migration #014): where it's bought (the store-grouped list), a deadline (urgency sort), a pause-until date (view-layer snooze — derivations stay clock-free). Deliberately do NOT bump `updated_at` (it's restock inference's since-bound; metadata is not a stock event). Never touch money |
| `change_password` | UI (session only) | member exists & active; current password verifies; new ≥ 8 chars and differs | The first write verb over `members`. Replaces the werkzeug hash; the audit row carries only the member id — never a password or hash. Route rate-limits wrong current passwords (5 / 15 min). No session revocation (signed cookies; later increment if wanted) |
| `merge_category` | UI | both names non-empty (into normalized: strip, ≤ 60); `from` ≠ `into`; neither is `Settlement`; `from` referenced somewhere | Categories are emergent tags, so "delete" = relabel every reference atomically (transactions all months, bills, pantry items; a budget follows the merge or retires if `into` has its own) — the caller must name a destination, which is what prevents orphans. Rename = merge into a fresh name. A relabel changes no money number |
...
| `set_item_interval` | UI | staple exists & active; `days` is None (clear) or an integer 1..365 | Sets/clears a staple's manual restock cadence ("remind me every N days", INVENTORY-DESIGN step 5); when set, `restock_forecast` counts N days from `last_stocked_at` instead of inferring the median from purchase history; None clears it → back to the inferred cadence; audit before/after. Never touches money |
